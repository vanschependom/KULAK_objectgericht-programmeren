...
class MoneyAmount implements Comparable<MoneyAmount> {
     ...
     * @param  numeral
     *         The numeral for this new money amount.
     * @effect This new money amount is initialized with the
     *         given numeral and the currency EUR.
     *       | this(numeral,Currency.EUR)
     */
    @Raw
    public MoneyAmount(BigDecimal numeral) throws IllegalArgumentException {
        this(numeral, Currency.EUR);
    }
     ...
     *
     * @return The money amount EUR_0 is equal to a money amount
     *         initialized with numeral BigDecimal.ZERO and with
     *         currency Currency.EUR.
     *       | EUR_0.equals(new MoneyAmount
     *       |     (BigDecimal.ZERO,Currency.EUR))
     */
    public final static MoneyAmount EUR_0 = new MoneyAmount(BigDecimal.ZERO,
            Currency.EUR);

    /**
     * Variable referencing a money amount of 1.00 EUR;
     *
     * @return The money amount EUR_1 is equal to a money amount
     *         initialized with numeral BigDecimal.ONE and with
     *         currency Currency.EUR.
     *       | EUR_1.equals(new MoneyAmount
     *       |     (BigDecimal.ONE,Currency.EUR))
     */
    public final static MoneyAmount EUR_1 = new MoneyAmount(BigDecimal.ONE,
            Currency.EUR);
     ...
     * @return True if and only if the given numeral is effective
     *         and if that numeral has a scale of 2.
     *       | result ==
     *       |   ( (numeral != null)
     *       |   && (numeral.scale() == 2) )
     */
     ...
     * @return The numeral of the resulting money amount is equal
     *         to the numeral of this money amount multiplied
     *         with the exchange rate from the currency of this
     *         money amount to the given currency rounded half-
     *         down to a scale of 2.
     *       | let
     *       |   exchangeRate =
     ...
     *       |        getContextForScale2(numeralInCurrency))
     * @throws IllegalArgumentException
    ...
    public MoneyAmount toCurrency(Currency currency)
        ...
        if (this.getCurrency() == currency)
            return this;
        BigDecimal exchangeRate = this.getCurrency().toCurrency(currency);
        BigDecimal numeralInCurrency = getNumeral().multiply(exchangeRate);
        numeralInCurrency = numeralInCurrency
                .round(getContextForScale2(numeralInCurrency));
     ...
     * @return The numeral of the resulting money amount is equal
     *         to the negation of the numeral of this money
     *         amount.
     *        | result.getNumeral().equals(
     *        |     this.getNumeral().negate())
     */
    public MoneyAmount negate() {
        return new MoneyAmount(this.getNumeral().negate(), getCurrency());
    }

    /**
     * Compute the product of this money amount with the given
     * factor.
     *
     * @param  factor
     ...
     *       | result.getCurrency() == this.getCurrency()
     * @return The numeral of the resulting money amount is equal
     *         to the product of the numeral of this money amount
    ...
    public MoneyAmount times(long factor) {
        ...
        return new MoneyAmount(this.getNumeral().multiply(factorDec),
                getCurrency());
    }
     ...
     *         as this money amount.
     *       | result.getCurrency() == this.getCurrency()
     * @return If the two money amounts use the same currency,
     *         the numeral of the resulting money amount is equal
     *         to the sum of the numerals of both money amounts.
     *       | if (this.getCurrency() == other.getCurrency())
     *       |   then result.getNumeral().equals
     *       |     this.getNumeral().add(other.getNumeral())
     * @return If the two money amounts use different currencies,
    ...
    public MoneyAmount add(MoneyAmount other) throws IllegalArgumentException {
        ...
        if (getCurrency() == other.getCurrency())
            return new MoneyAmount(getNumeral().add(other.getNumeral()),
                    getCurrency());
    ...
    public MoneyAmount subtract(MoneyAmount other)
        ...
        if (other == null)
            throw new IllegalArgumentException("Non-effective money amount");
        if (getCurrency() == other.getCurrency())
            return new MoneyAmount(getNumeral().subtract(other.getNumeral()),
                    getCurrency());
    ...
    public int compareTo(MoneyAmount other) throws ClassCastException {
        ...
        if (getCurrency() != other.getCurrency())
            throw new ClassCastException("Incompatable money amounts");
     ...
     * @param  other
     *         The other money amount to compare with.
     * @return True if and only if this money amount is equal to
     *         the other money amount expressed in the currency
     ...
     *       | other == null
     */
    public boolean hasSameValueAs(MoneyAmount other) throws ClassCastException {
        if (other == null)
            throw new IllegalArgumentException("Non-effective money amount");
     ...
     *         if this money amount and the given object belong
     *         to the same class, and if this money amount and
     *         the other object interpreted as a money amount
     *         have equal numerals and equal currencies.
     *        | result ==
     *        |   ( (other != null)
    ...
    public boolean equals(Object other) {
        ...
        return (this.getNumeral().equals(otherAmount.getNumeral()) && (this
                .getCurrency() == otherAmount.getCurrency()));
    }
