 ...
 * a holder, a credit limit, a balance, a blocking facility
 * and a bank card.
 *
 ...
 * @invar   The holder of each bank account must be a valid
 *          holder for a bank account.
 ...
 * @invar   Each bank account must have a proper bank card
 *          attached to it.
 *        | hasProperBankCard()
 *
 * @version 2.0
...
public class BankAccount {
	 ...
	 * holder, given credit limit, given balance, given blocked
	 * state and no bank card attached to it.
	 *
	 * @param  number
	 ...
	 * @effect The holder for this new bank account is set to
	 *         the given holder.
	 *       | setHolder(holder)
	 ...
	 * @post  This new bank account has no bank card attached to
	 *        it yet.
	 *      | ! new.hasBankCard()
	 * @post  This new bank account is not yet terminated.
	 *      | ! new.isTerminated()
	 ...
	 */
	@Raw
	public BankAccount(int number, Person holder, MoneyAmount creditLimit,
			MoneyAmount balance, boolean isBlocked)
			throws IllegalNumberException, IllegalHolderException,
	 ...
	 * with given number, given holder, with a credit limit of
	 * -1000,00 EUR, with a balance of 0,00 EUR and with no bank
	 * card attached to it yet.
	 *
	 * @param  number
	 *         The number for this new bank account.
	 * @param  holder
	 ...
	 *         given number as its number, with the given holder
	 *         as its holder, with -1000 EUR as its credit limit,
	 *         with 0 EUR as its balance, and with false as its blocked state.
	 *       | this(number,
	 *       |      new MoneyAmount(new BigDecimal(-1000)), holder,
	 *       |      MoneyAmount.EUR_0,false)
	 */
	@Raw
	public BankAccount(int number, Person holder)
			throws IllegalNumberException, IllegalHolderException {
		this(number, holder, new MoneyAmount(new BigDecimal(-1000)),
				MoneyAmount.EUR_0, false);
	}

	/**
	 * Terminate this bank account and its associated bank card,
	 * if any.
	 *
	 * @post  This bank account is terminated.
	 *      | new.isTerminated()
	 * @post  No bank card is attached to this bank account.
	 *      | ! new.hasBankCard()
	 * @post  The bank card attached to this account, if any,
	 *        is terminated.
	 *      | if (hasBankCard())
	 *      |   then (new getBankCard()).isTerminated()
	 */
	...
	/**
	 * Variable registering whether or not this bank account is
	 * terminated.
	 */
	...
	/**
	 * Check whether the given person is a valid holder for any
	 * bank account.
	 *
	 * @param  person
	 *         The person to check.
	 * @return True if and only if the given person is an
	 *         effective and adult person.
	 ...
	 * @post   The new holder for this bank account is the same
	 *         as the given holder.
	 *       | new.getHolder() == holder
	 * @throws IllegalHolderException(holder,this)
	...
	*/
	...
	private boolean isBlocked = false;
	  ...
	  /**
	   * Return the bank card attached to this bank account.
	   *   A null reference is returned if no card is attached.
	   */
	  @Basic @Raw
	  ...
	  /**
	   * Check whether this bank account has a bank card attached
	   * to it.
	   *
	   * @return True if and only if this bank account references
	   *         an effective bank card.
	  ...
	  /**
	   * Check whether this bank account has a proper bank card
	   * attached to it.
	   *
	   * @return True if and only if this bank account does not
	   *         reference an effective bank card, or if the bank
	   *         card referenced by this bank account in turn
	   *         references this bank account as the account to
	   *         which it is attached.
	   *       | result ==
	   *       |   ( (getBankCard() == null)
	   *       |  || (getBankCard().getAccount() == this) )
	   */
	  @Raw
	  ...
	  /**
	   * Set the bank card attached to this bank account to the
	   * given card.
	   *
	   * @param  card
	   *         The bank card to be attached to this bank account.
	   * @pre    If the given card is effective, it must already
	   *         reference this bank account as its bank account.
	   *       | if (card != null)
	   *       |   then card.getAccount() == this
	   * @pre    If the given card is not effective and this bank
	   *         account has a bank card attached to it, that
	   *         bank card may not reference this bank account
	   *         as the account to which it is attached.
	   *       | if ( (card == null) && this.hasBankCard() )
	   *       |   then (getBankCard().getAccount() != this)
	   * @post   This bank account references the given card as the
	   *         bank card attached to it.
	  ...
	  public void setBankCard(@Raw BankCard card) {
	    ...
	    assert
	       ( (card != null) || (! hasBankCard())
	      || (getBankCard().getAccount() == null) );
	    this.bankCard = card;
	  }

	  /**
	   * Variable referencing the bank card attached to this bank
	   * account.
	   */
